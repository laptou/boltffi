...
fn emit_writer_write_op(op: &WriteOp, w: &str, root_value: &str) -> String {
    match op {
        ...
        WriteOp::Option { value, some } => {
            let some_rebased = remap_root_in_seq(some, ValueExpr::Var("v".into()));
            let inner = emit_writer_write(&some_rebased, w, "v");
            format!(
        ...
        WriteOp::Vec {
            ...
            let element_rebased = remap_root_in_seq(element, ValueExpr::Var("item".into()));
            let inner = emit_writer_write(&element_rebased, w, "item");
    ...
    match size {
        ...
        SizeExpr::VecSize {
            ...
            match layout {
                ...
                VecLayout::Encoded => {
                    let inner_size = emit_size_expr(
                        &remap_size_root(inner, ValueExpr::Var("item".into())),
                        "item",
                    );
