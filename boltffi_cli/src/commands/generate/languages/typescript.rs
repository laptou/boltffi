use std::path::Path;

use boltffi_bindgen::render::typescript::{
    TypeScriptEmitter, TypeScriptLowerError, TypeScriptLowerer,
};

use crate::cli::{CliError, Result};
use crate::commands::generate::generator::{GenerateRequest, LanguageGenerator, ScanPointerWidth};
use crate::config::{Target, WasmNpmTarget};

pub struct TypeScriptGenerator;

impl LanguageGenerator for TypeScriptGenerator {
    const TARGET: Target = Target::TypeScript;

    fn generate(request: &GenerateRequest<'_>) -> Result<()> {
        if !request.config().is_wasm_enabled() {
            return Err(CliError::CommandFailed {
                command: "targets.wasm.enabled = false".to_string(),
                status: None,
            });
        }

        let output_directory = request
            .output_override()
            .map(ToOwned::to_owned)
            .unwrap_or_else(|| request.config().wasm_typescript_output());
        let module_name = request.config().wasm_typescript_module_name();
        let browser_output_path = output_directory.join(format!("{module_name}.ts"));
        let node_output_path = output_directory.join(format!("{module_name}_node.ts"));

        request.ensure_output_directory(&output_directory)?;

        let lowered_crate = request.lowered_crate(
            ScanPointerWidth::Fixed(32),
            Some(request.config().wasm_scan_cfg_context()),
        )?;

        let mut type_script_module = TypeScriptLowerer::new(
            &lowered_crate.ffi_contract,
            &lowered_crate.abi_contract,
            request.source_crate().crate_name().to_string(),
            request.config().typescript_experimental(),
        )
        .lower()
        .map_err(|error| match error {
            TypeScriptLowerError::ValueTypeMemberNameCollision { .. }
            | TypeScriptLowerError::TopLevelFunctionNameCollision { .. } => {
                CliError::CommandFailed {
                    command: format!("generate typescript: {error}"),
                    status: None,
                }
            }
        })?;
        if let Some(glue) = read_wasm_bindgen_glue_marker(&output_directory, &module_name) {
            type_script_module.wasm_bindgen_glue = Some(glue.clone());
            write_wasm_bindgen_glue_dts(&output_directory, &glue, request)?;
        }
        let runtime_package = request.config().wasm_runtime_package();
        let emits_node_bundle = request
            .config()
            .wasm_npm_targets()
            .iter()
            .any(|target| matches!(target, WasmNpmTarget::Nodejs));

        let browser_source = TypeScriptEmitter::emit(&type_script_module).replacen(
            "from \"@boltffi/runtime\"",
            &format!("from \"{}\"", runtime_package),
            1,
        );

        request.write_output(&browser_output_path, browser_source)?;

        if emits_node_bundle {
            let node_source = TypeScriptEmitter::emit_node(&type_script_module, &module_name)
                .replacen(
                    "from \"@boltffi/runtime\"",
                    &format!("from \"{}\"", runtime_package),
                    1,
                );
            request.write_output(&node_output_path, node_source)?;
        } else if node_output_path.exists() {
            std::fs::remove_file(&node_output_path).map_err(|source| CliError::WriteFailed {
                path: node_output_path.clone(),
                source,
            })?;
        }

        Ok(())
    }
}

/// `boltffi pack wasm` writes `{module}.boltffi.json` when wasm-bindgen glue is present.
fn read_wasm_bindgen_glue_marker(output_dir: &Path, module_name: &str) -> Option<String> {
    let path = output_dir.join(format!("{module_name}.boltffi.json"));
    let data = std::fs::read(path).ok()?;
    let v: serde_json::Value = serde_json::from_slice(&data).ok()?;
    v.get("wasm_bindgen_glue")
        .and_then(|x| x.as_str())
        .map(|s| s.to_string())
}

/// Emits a minimal `.d.ts` for the wasm-bindgen glue JS file so TypeScript consumers of the
/// generated bindings don't get TS7016 "could not find declaration file" errors.
///
/// Only the two symbols imported by BoltFFI-generated TypeScript are declared here.
fn write_wasm_bindgen_glue_dts(
    output_dir: &Path,
    glue_js: &str,
    request: &GenerateRequest<'_>,
) -> Result<()> {
    let dts_name = wasm_bindgen_glue_dts_name(glue_js);
    let dts_path = output_dir.join(&dts_name);
    request.write_output(&dts_path, wasm_bindgen_glue_dts_content())
}

fn wasm_bindgen_glue_dts_name(glue_js: &str) -> String {
    if let Some(stem) = glue_js.strip_suffix(".js") {
        format!("{stem}.d.ts")
    } else {
        format!("{glue_js}.d.ts")
    }
}

/// minimal declarations covering the two symbols imported by boltffi-generated typescript
fn wasm_bindgen_glue_dts_content() -> String {
    // __wbg_get_imports must return WebAssembly.Imports (record<string, ...>), not bare object.
    // __wbg_finalize_init receives an instantiated wasm module and returns its export object.
    "// generated by boltffi — declarations for wasm-bindgen glue\n\
        export declare function __wbg_get_imports(): WebAssembly.Imports;\n\
        export declare function __wbg_finalize_init(\n  \
            instance: WebAssembly.Instance,\n  \
            module: WebAssembly.Module,\n\
        ): object;\n"
        .to_string()
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn glue_dts_name_replaces_js_extension() {
        assert_eq!(
            wasm_bindgen_glue_dts_name("current_core_wbg.js"),
            "current_core_wbg.d.ts"
        );
    }

    #[test]
    fn glue_dts_name_appends_dts_when_no_js_suffix() {
        assert_eq!(
            wasm_bindgen_glue_dts_name("current_core_wbg"),
            "current_core_wbg.d.ts"
        );
    }

    #[test]
    fn glue_dts_content_declares_get_imports_and_finalize_init() {
        let content = wasm_bindgen_glue_dts_content();
        assert!(
            content.contains("export declare function __wbg_get_imports"),
            "must declare __wbg_get_imports"
        );
        assert!(
            content.contains("WebAssembly.Imports"),
            "__wbg_get_imports must return WebAssembly.Imports"
        );
        assert!(
            content.contains("export declare function __wbg_finalize_init"),
            "must declare __wbg_finalize_init"
        );
        assert!(
            content.contains("WebAssembly.Instance"),
            "finalize_init must reference WebAssembly.Instance"
        );
        assert!(
            content.contains("WebAssembly.Module"),
            "finalize_init must reference WebAssembly.Module"
        );
    }
}
