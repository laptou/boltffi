...
/// `Some(())` when `rust_type` is a resolved `Box<dyn Trait>` / `Arc<dyn Trait>` (etc.) callback return.
pub(crate) fn try_resolve_callback_handle_return(
    rust_type: &Type,
    callback_registry: &CallbackTraitRegistry,
) -> syn::Result<Option<()>> {
    ...
    match descriptor.resolve(rust_type, callback_registry)? {
        Some(_) => Ok(Some(())),
        None => Ok(None),
    }
}
