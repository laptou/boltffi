        ...
        .filter_map(|exportable_method| {
            ...
            match (callable.form(), callable.execution_kind()) {
                ...
                (CallableForm::StaticMethod, ExecutionKind::Async) => {
                    generate_async_static_method_export(
                        callable,
                        &type_name,
                        &type_name_str,
                        &return_lowering,
...
fn generate_async_static_method_export(
    callable: MethodCallable<'_>,
    type_name: &syn::Ident,
    class_name: &str,
    return_lowering: &ReturnLoweringContext<'_>,
    ...
    // align async static export names with sync factory constructors: `new` maps to `*_new`, not `*_new_new`.
    let base_name = if is_factory_constructor(method, type_name) {
        if method_name_str == "new" {
            naming::class_ffi_new(class_name)
        } else {
            naming::method_ffi_name(class_name, &method_name_str)
        }
    } else {
        naming::method_ffi_name(class_name, &method_name_str)
    };
    ...
    let future_body = quote! {
        #(#thread_setup)*
        #type_name::#method_name(#(#call_args),*).await
    };
